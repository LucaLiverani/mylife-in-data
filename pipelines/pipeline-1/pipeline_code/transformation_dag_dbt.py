...
with DAG(
    dag_id="transformation_dag_dbt",
    default_args=default_args,
    description="Run dbt transformations for all data sources (Spotify, Google Maps, etc.)",
    schedule="*/15 * * * *",  # Run every 15 minutes
    start_date=pendulum.datetime(2025, 1, 1, tz="UTC"),
    catchup=False,
    max_active_runs=1,
    tags=["dbt", "transformation", "spotify", "google-maps"],
) as dag:
    ...
    # DAG dependencies
    start >> fix_permissions >> dbt_deps >> dbt_run >> dbt_test >> complete
