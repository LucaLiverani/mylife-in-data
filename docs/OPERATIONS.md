...
### Stream schema contract (spotify.player.current)

The now-playing stream is the one place where a malformed message can vanish without a trace (`kafka_skip_broken_messages = 1` on the Kafka-engine table), so it carries an explicit contract:

- **The contract** is `ingestion/spotify/schemas/spotify_player_current.v1.json` (JSON Schema draft-07, closed and fully required: a renamed/added/retyped field is a violation). At startup the producer registers it in Redpanda's Schema Registry (`SCHEMA_REGISTRY_URL`, default `http://redpanda:8081`) under subject `spotify.player.current-value` and pins the subject to `BACKWARD` compatibility. Registration is best-effort by design: the ClickHouse consumer reads plain JSONEachRow (the Confluent wire format would corrupt it), so **enforcement is producer-side against the local file**; the registry is the versioned, compatibility-checked contract store, browsable in Redpanda Console (`:8090`).
- **Validation happens on the wire form** (the event after `json_utils.dumps`), so serializer regressions are caught too: the schema's `captured_at` pattern encodes the basic `YYYY-MM-DD HH:MM:SS.mmm` form precisely because the ISO `…T…Z` form once silently emptied the table.
- **Rejects go to the dead-letter topic** `spotify.player.current.dlq` wrapped in a `{rejected_at, topic, error, payload}` envelope, which lands in `bronze.spotify_player_current_dlq` (and rides the nightly R2 archive). They also log as ERROR in the producer. Inspect with:

  ```sql
  SELECT rejected_at, error, payload
  FROM bronze.spotify_player_current_dlq ORDER BY rejected_at DESC LIMIT 20;
  ```

  or `docker exec redpanda rpk topic consume spotify.player.current.dlq`. A healthy stream has zero rows here.
- **Evolving the contract**: change the producer, the schema file, and the Kafka-engine DDL together; CI (`scripts/check_stream_contract.py` in `backend-validate`) fails if any pair drifts. On deploy the producer restarts and re-registers; a compatible change becomes a new registry version, an incompatible one is refused (HTTP 409, logged as ERROR) while producing continues against the local schema. Either evolve compatibly or deliberately relax the subject (`curl -X PUT localhost:18081/config/spotify.player.current-value -H 'Content-Type: application/vnd.schemaregistry.v1+json' -d '{"compatibility":"NONE"}'`), then restart the producer.
- `kafka_skip_broken_messages = 1` stays on both Kafka-engine tables as the last-resort guard for messages that bypass the producer (e.g. hand-produced garbage); everything the producer emits is either schema-valid or in the DLQ.
...
- **Spotify "Now playing" shows an idle state when nothing's playing.** The producer emits on track change / play-pause flip (and the current track on startup), polling `current_playback()` with a fallback to `current_user_playing_track()` so it captures playback even when Spotify reports no "active device". When nothing's playing, `bronze.spotify_player_current` simply holds the last transition and the tile reads "the studio is quiet" — the `/api/spotify/current` mock is `no_track`, not a fake track. **Timestamp gotcha:** the producer must emit `captured_at` in ClickHouse's basic `YYYY-MM-DD HH:MM:SS.mmm` form; ISO `…T…Z` strings are unparseable to the Kafka engine and once left the table permanently empty via `kafka_skip_broken_messages=1`. The schema contract now encodes that form (see "Stream schema contract" above), so a regression lands loudly in `bronze.spotify_player_current_dlq` instead of vanishing. Also: `tokens/` is bind-mounted read-only, so spotipy can't persist *refreshed* tokens (harmless repeating "Couldn't write token to cache" warnings) — fine until the refresh token rotates, at which point re-auth on the VM (`authenticate_local.py`).
